...
class RecordingForegroundService : LifecycleService() {
    ...
    private fun startLockLensRecording(intent: Intent) {
        ...
        startForeground(
            NOTIFICATION_ID,
            buildNotification("Подготовка записи...", true)
        )
        sendState(STATE_PREPARING)
        ...
        providerFuture.addListener(
            {
                try {
                    ...
                    var pendingRecording: PendingRecording =
                        recorder.prepareRecording(this, outputOptions)
                    ...
                    activeRecording = pendingRecording.start(
                        ...
                        when (event) {
                            is VideoRecordEvent.Start -> {
                                updateNotification("Идёт запись. Нажмите Стоп для сохранения.")
                                sendState(STATE_RECORDING)
                            }
                    ...
                    sendState(
                        state = STATE_FAILED,
                        message = error.message ?: "Не удалось запустить камеру"
                    )
                    cleanupAfterRecording()
                }
            },
    ...
    private fun stopLockLensRecording() {
        ...
        updateNotification("Сохраняем запись...")
